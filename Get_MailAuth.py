...
def get_dns_txt_record(domain: str, record: str = "") -> str:
    """
    使用 nslookup 獲取 DNS TXT 紀錄，並提取 v=spf1, v=DMARC1, v=DKIM1 等相關設定
    ...
    try:
        ...
        # 提取 v=spf1, v=DMARC1, v=DKIM1 到下一個雙引號的內容
        patterns = [r"(v=spf1[^\"']*)", r"(v=DMARC1[^\"']*)", r"(v=DKIM1[^\"']*)"]
        extracted_records = []
        
        for pattern in patterns:
            match = re.search(pattern, result.stdout)
            if match:
                extracted_records.append(match.group(1))
        
        # 如果沒有找到任何相關資訊，回傳 "沒有找到 TXT 紀錄。"
        return "\n".join(extracted_records) if extracted_records else "沒有找到 TXT 紀錄。"
